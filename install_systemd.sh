...
pacman -S --noconfirm --needed reflector rsync arch-install-scripts git ntp wget systemd ntpd networkmanager linux-lts
...
if [[ -d "/sys/firmware/efi" ]]; then
    bootctl install
    echo -e "Systemd-boot installed."
else
    echo "ERROR: Systemd-boot requires UEFI. Check your boot mode."
    exit 1
fi
...
pacman -S --noconfirm --needed reflector rsync arch-install-scripts git ntp wget
...
KERNEL_VERSION=$(ls /usr/lib/modules/ | tail -n 1)
...
if [[ "${FS}" == "luks" ]]; then
    sed -i "s%options root=[^ ]*%options cryptdevice=UUID=${ENCRYPTED_PARTITION_UUID}:ROOT root=/dev/mapper/ROOT rw%" /boot/loader/entries/arch.conf
fi
...
-------------------------------------------------------------------------
"
pacman -S --needed --noconfirm fastfetch firefox
